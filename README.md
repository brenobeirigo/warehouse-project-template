...
them, generates a zone summary, chart, and LaTeX tables, and builds an IEEE-style PDF.
...
`artifacts/scenario_inventory_factor.csv`, the chart and LaTeX tables under
`artifacts/`, and `artifacts/main.pdf`. Open the PDF to see the three
generated tables and chart.
...
flowchart LR
    ...
    F --> G[Write CSV, chart, and LaTeX tables]
...
`report/main.tex` imports the generated tables, chart, and summary text. The
PDF therefore depends on the pipeline output. The `all` command regenerates
these files each time.

Run `run.cmd scenario --factor 1.10` on Windows, or
`./run.sh scenario --factor 1.10` on macOS or Linux, for a small command example. It writes
...
section and gives examples for settings tables, comparisons, results,
limitations, and recommendations. It adapts the author's writing and table
examples and the public
[LaTeX thesis template](https://github.com/brenobeirigo/latex-thesis-template).
The working examples here are complete, so students do not need access to the
author's private repositories. The
...
| `artifacts/` | Generated CSV, chart, three LaTeX tables, run record, and PDF; excluded from Git |
