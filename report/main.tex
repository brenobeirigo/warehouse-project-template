...
Table~\ref{tab:example-settings} records the input scope and calculation
choices behind this example. Group related settings and state the value used
in the reported run. For a simulation, add the demand scenarios, seeds,
replications, warm-up period, stopping rule, solver version, and tolerances
that affect its results.

\begin{table}[!ht]
\caption{Settings used to generate the inventory example; the multiplier is
an arithmetic illustration, not an estimated demand change.}
\label{tab:example-settings}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
\input{artifacts/settings_table.tex}
\end{table}
...
\subsection{A result comparison example}
Table~\ref{tab:scenario-example} shows a comparison layout with a common unit,
an observed baseline, an illustrative value, and an absolute change. The
pipeline calculates every cell from the checked source and the stated factor.
The grouped header states the unit once, and right-aligned numbers make the
differences easier to inspect.

\begin{table}[H]
\caption{Observed and illustrative selling units by zone in the DC23ACTIVE
snapshot ($n=\activeLocationCount$ location records); the illustrative column applies the factor in
Table~\ref{tab:example-settings} and rounds each zone up to whole units.}
\label{tab:scenario-example}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
\input{artifacts/scenario_table.tex}
\end{table}

\input{artifacts/scenario_summary.tex}

\emph{Student prompt: Replace this arithmetic example with a tested
intervention. Name the baseline, metric, unit, absolute and relative effect,
and uncertainty. Explain the main result in prose instead of repeating every
cell. Keep the same conditions across options. Split a crowded table instead
of shrinking its text.}
